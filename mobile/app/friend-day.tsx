...
export default function FriendDayScreen() {
  ...
  // NOTE: a react-native-screens `formSheet` expects its content to be at most
  // two subviews (an optional header + the ScrollView). Wrapping these in a
  // plain <View> gets flattened and makes RNScreens miscount the subviews, which
  // collapses the ScrollView to height 0 on the second presentation (blank
  // sheet). Render the ScrollView as the direct content and mark the overlaid
  // close button `collapsable={false}` so it is recognized as the header.
  return (
    <>
      <ReadOnlyDayView
        view={friendDayView}
